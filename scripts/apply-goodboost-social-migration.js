...
const LOCK_NAME = "goodbase:migration:goodboost-production-v1";
...
function ready(state) {
  return Object.values(state).length === 7 && Object.values(state).every(Boolean);
}
...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({ migration: "goodboost-production-v1", status: ready(before) ? "verified" : "applied", schema: after }));
  } finally {
