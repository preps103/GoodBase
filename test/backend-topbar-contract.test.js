...
test("versioned GoodOS UI widget packages are delivered as immutable shared assets", () => {
  const routes = read("src/routes/index.js");
  const packagePaths = ["3.0.0", "4.1.0", "4.2.0", "4.3.0", "4.4.0", "4.5.0", "4.6.0", "4.7.0", "4.8.0", "4.9.0"].map((version) =>
    path.join(
      root,
  ...
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.3\.0\.tgz"/);
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.4\.0\.tgz"/);
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.5\.0\.tgz"/);
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.6\.0\.tgz"/);
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.7\.0\.tgz"/);
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.8\.0\.tgz"/);
  assert.match(routes, /router\.get\("\/packages\/goodos-topbar-widget-4\.9\.0\.tgz"/);
