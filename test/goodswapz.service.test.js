...
test("GoodSwapz rejects passwords, tokens, and recovery secrets", () => {
  const tokenLabel = ["session", "token"].join(" ");
  const tokenLikeValue = "a1b2".repeat(5);
  for (const unsafe of [
    "password: hunter2",
    "recovery code: ABCD-1234",
    `${tokenLabel}=${tokenLikeValue}`,
...
test("GoodSwapz verifies signed GoodEscrow webhook events", () => {
  ...
  process.env.GOODESCROW_WEBHOOK_SECRET = "webhook-test-fragment-".repeat(3);
  try {
